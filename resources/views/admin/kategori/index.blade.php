...
@section('content')
    ...
    <div id="deleteKategoriModal">
        <div class="delete-modal-box">
            <button onclick="closeDeleteKategoriModal()" style="position: absolute; top: 1rem; right: 1rem; background: none; border: none; font-size: 1.5rem; cursor: pointer; color: #64748B; line-height: 1;">&times;</button>
            <div style="text-align: center;">
                ...
                <p style="color: #64748B; font-size: 0.875rem; margin: 0 0 1.5rem;">Apakah Anda yakin ingin menghapus kategori ini? Tindakan ini tidak dapat dibatalkan.</p>
                <div style="display: flex; justify-content: center; gap: 1rem;">
                    <button type="button" onclick="closeDeleteKategoriModal()" style="padding: 0.625rem 1.5rem; border-radius: 8px; font-weight: 500; background: #F1F5F9; color: #475569; border: 1px solid #E2E8F0; cursor: pointer;">Batal</button>
                    <button type="button" onclick="confirmDeleteKategori()" style="background: #DC2626; color: white; padding: 0.625rem 1.5rem; border-radius: 8px; font-weight: 500; border: none; cursor: pointer;">Ya, Hapus</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    async function confirmDeleteKategori() {
        ...
        try {
            const response = await fetch(`{{ route("admin.kategori.destroy", ":id") }}`.replace(':id', deleteKategoriId), {
                method: 'DELETE',
                headers: {
