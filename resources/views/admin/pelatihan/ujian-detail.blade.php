...
<script>
    ...
    // Variable untuk menyimpan ID soal yang akan dihapus
    let deleteSoalIdUjian = null;

    function deleteSoalConfirm(soalId, pertanyaan) {
        deleteSoalIdUjian = soalId;
        const previewText = pertanyaan ? pertanyaan.substring(0, 80) + (pertanyaan.length > 80 ? '...' : '') : '';
        document.getElementById('deleteSoalText').innerHTML = previewText 
            ? `Apakah Anda yakin ingin menghapus soal:<br><strong>"${previewText}"</strong>?`
            : 'Apakah Anda yakin ingin menghapus soal ini?';
        document.getElementById('modalDeleteSoal').classList.add('active');
        document.body.style.overflow = 'hidden';
    }

    function closeDeleteSoalModal() {
        document.getElementById('modalDeleteSoal').classList.remove('active');
        document.body.style.overflow = '';
        deleteSoalIdUjian = null;
    }

    function confirmDeleteSoalUjian() {
        if (!deleteSoalIdUjian) return;
        
        fetch(`/admin/soal/${deleteSoalIdUjian}`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json'
...
<!-- Modal Konfirmasi Hapus Soal -->
<div id="modalDeleteSoal" class="modal-overlay">
    <div class="modal-container-soal" style="max-width: 380px;">
        <div style="padding: 1.75rem; text-align: center;">
            ...
            <p id="deleteSoalText" style="font-size: 0.8125rem; color: #64748B; margin-bottom: 0.75rem;">Apakah Anda yakin ingin menghapus soal ini?</p>
            <p style="font-size: 0.7rem; color: #EF4444; margin-bottom: 1.5rem;">Tindakan ini tidak dapat dibatalkan</p>
            
            <div style="display: flex; gap: 0.75rem; justify-content: center;">
                <button type="button" onclick="closeDeleteSoalModal()" style="flex: 1; max-width: 120px; padding: 0.5rem 1rem; font-size: 0.875rem; background: #F3F4F6; color: #374151; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; transition: all 0.2s;">Batal</button>
                <button type="button" onclick="confirmDeleteSoalUjian()" style="flex: 1; max-width: 120px; padding: 0.5rem 1rem; font-size: 0.875rem; background: #DC2626; color: white; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; transition: all 0.2s;">Ya, Hapus</button>
            </div>
        </div>
    </div>
</div>
